...
class BenToJSON {
  ...
  constructor(data) {
    ...
    this.json = {};
    // List of keys to bad UTF8 encoded strings
    this.invalid_utf8 = [];
    ...
    // record errors
    if (this.invalid_utf8.length > 0) {
      this.json.error = { invalid_utf8: this.invalid_utf8 };
    }
  }
  ...
  skip_arrows() {
    if (this.gv_version >= 3) {
      ...
      this.ole.skip(num_curved_arrows * 4 * 23);
    }
  }
  ...
  read_timestamps() {
    if (this.version_ge(3) && this.json.meta.has_timestamps) {
      this.json.data_set.forEach((ds) => {
        ds.timestamp = [];
        for (let i = 0; i < ds.no_of_points; ++i ) {
          ds.timestamp.push(this.ole.read_systemtime());
        }
      });
    }
  }
  ...
  */
  ...
  read_spectral_markers() {
    ...
    for (let i = 0; i < no_of_values; ++i) {
      ...
      marker.x_value = this.ole.read_double();
      marker.y_value = this.ole.read_double();
    ...
    if (count > 0) {
      for (let i = 0; i < count; ++i) {
        ...
        for (let j = 0; j < count2; ++j) {
          ...
          measurement_info.measurement = this.ole.read_utf8_string();
          measurement_info.units = this.ole.read_utf8_string();
    ...
    if (count > 0) {
      for (let i = 0; i < count; ++i) {
        ...
        for (let j = 0; j < count2; ++j) {          
          ...
          const x_value = this.ole.read_double_array(no_of_values);
          const y_value = this.ole.read_double_array(no_of_values);
          const uncertain = this.ole.read_bool_array(no_of_values);
          
          const info = {};
          info.points = no_of_points;
          info.x_measurement = this.ole.read_utf8_string();
          info.x_units = this.ole.read_utf8_string();
          info.y_measurement = this.ole.read_utf8_string();
          info.y_units = this.ole.read_utf8_string();
